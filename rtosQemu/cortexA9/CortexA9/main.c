...
static void vCheckTask( void *pvParameters )
{
	...
	for( ;; )
	{
		...
		{
			sprintf( (char *)cBuffer, "Pass: %lu, Core: %ld\r\n", xLastExecutionTime / 1000, portCORE_ID() );
...
static unsigned long prvReadP15_C0_C0_0( void )
{/* MIDR; 0x1091551376 */
	volatile unsigned long ulReturn = 0UL;
	__asm volatile(
		" mrc p15, 0, %[retval], c0, c0, 0 			\n"
		: [retval] "=r" (ulReturn) : :
	);
	return ulReturn;
}
...
{/* Coprocessor Access Control Register, p4-82 */
	volatile unsigned long ulReturn = 0UL;
	/* Read Configuration Register C15, c0, 0. */
	__asm volatile(
...
char cAddress[32];
	...
	vSerialPutString((xComPortHandle)configUART_PORT,"Init\r\n", 6);

	sprintf( cAddress, "MID: %ld\r\n", prvReadP15_C0_C0_0() );
	...
	/*stop here when platform=2  */
	/* following 4 registers :accesses in Non-secure state cause an Undefined Instruction exception */
#if 0
	ulValue = prvReadP15_C1_C1_0(); /* Secure Configuration Register */
	sprintf( cAddress, "SCR: 0x%08lX\r\n", ulValue );
	vSerialPutString((xComPortHandle)configUART_PORT,(const signed char * const)cAddress, strlen(cAddress) );

	/* Read the Configuration Registers to find the vital information. */
	ulValue = prvReadP15_C1_C1_1(); /* Secure Debug Enable Register */
	sprintf( cAddress, "SDER: 0x%08lX\r\n", ulValue );
	vSerialPutString((xComPortHandle)configUART_PORT,(const signed char * const)cAddress, strlen(cAddress) );

	ulValue = prvReadP15_C1_C1_2();/* Non-secure Access Control Register */
	sprintf( cAddress, "NSACR: 0x%08lX\r\n", ulValue );
	vSerialPutString((xComPortHandle)configUART_PORT,(const signed char * const)cAddress, strlen(cAddress) );

	ulValue = prvReadP15_C12_C0_1(); /* Vector Base Address Register, p4-62 */
...
int main( void )
{
	...
	prvSetupHardware();
	vSerialPutString((xComPortHandle)mainPRINT_PORT, (const signed char * const)"start here...\r\n", 15 );
