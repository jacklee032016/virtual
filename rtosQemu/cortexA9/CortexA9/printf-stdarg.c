...
static char *formatinteger(char *bufferend,unsigned int absvalue, unsigned int base,char letterbase,int zeropadwidth,bool zeroisempty)
{
...
static int printstring(char **out,const char *string,int width,bool padleft)
{
	...
	// If we need to pad on the left, calculate the length of the string and print the appropriate amount of padding.
	if(width>0 && padleft)
	{
