...
void main(void)
{
	...
	*/
	...
	if ( pdPASS != xTaskCreate(printGateKeeperTask, "gk", 128, NULL, PRIOR_PRINT_GATEKEEPR, NULL) )
	{
	...
	if ( pdPASS != xTaskCreate(vTaskFunction, "task1", 128, (void*) &tParam[0], PRIOR_PERIODIC, NULL) )
	{
		FreeRTOS_Error("Could not create task1\r\n");
	}

	if ( pdPASS != xTaskCreate(vPeriodicTaskFunction, "task2", 128, (void*) &tParam[1], PRIOR_FIX_FREQ_PERIODIC, NULL) )
	{
